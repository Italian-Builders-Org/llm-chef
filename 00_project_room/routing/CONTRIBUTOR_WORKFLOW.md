...
- Adam reviews (with Codex/Claude as needed). Merges if it meets the success check; otherwise requests changes.
...
## Board setup recipe (Phase 0 — Adam, one-time)
...
If org-level Project or repo creation is restricted, ask an org owner (Domenico) — see the access asks in the project proposal.
