...
- ADR-0008 — Project licenses: code MIT, data CC0, content PD/CC0 — **Accepted**
...
| 2026-06-28 | IB-008/009/010: seed metadata title-page-verified; 15 pre-1929 candidates identified; owner signed off 6 to `public_domain_verified` under a bright-line rule (clear in both US + EU, no unconfirmed facts, no reprint/source flags). | Decided | Title-page evidence + death years (Pettini d.1948, Masoni d.1941, Staffe/Artusi d.1911; anonymous = 70y-from-publication). Hess 1912 blocked (EU © to 2035). | If a named author surfaces on a fuller title page, or a scan proves to be a reprint. |
...
| 2026-06-28 | IB-038 (ADR-0008, **Accepted**): code = **MIT** (`LICENSE` committed); data/metadata = CC0; PD-source Level A transcriptions = PD/CC0; future editorial (Level B/C) = CC-BY-SA reserved (V2). | Decided | MIT selected over Apache for simplicity; open-source intent + rights constraints (metadata is largely fact; faithful PD transcriptions carry no new copyright). | If the group forms a legal entity (amend `LICENSE` holder), or wants a protected lane for paid editorial work. |
| 2026-06-28 | V1 reader is transcription-only: the only public asset is the Level A transcription. No page-facsimile / "view original page" feature; master PDFs, raw OCR, and page images stay private. | Decided | Product surface is cleaned text (ADR-0004), not scans; serving images is unnecessary and keeps the asset model simple. | If a "view source" verification feature is later wanted for trust/research. |
