...
Currently configured project: `Chef-llm` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/Chef-llm/SETUP.md` to fill in
   `data/Chef-llm/INFO.md`.
...
`--project-id Chef-llm` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/Chef-llm/`.
