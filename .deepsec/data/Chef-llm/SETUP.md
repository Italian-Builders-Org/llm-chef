# Agent setup for `Chef-llm`

This is a deepsec scanning workspace. Project `Chef-llm` was just registered
(target: `..`). Setup is incomplete — `data/Chef-llm/INFO.md`
...
2. **Fill in `data/Chef-llm/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id Chef-llm
pnpm deepsec process --project-id Chef-llm
```
